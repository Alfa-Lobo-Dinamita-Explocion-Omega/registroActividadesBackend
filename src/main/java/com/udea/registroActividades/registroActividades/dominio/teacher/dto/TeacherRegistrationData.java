...
public record TeacherRegistrationData(
        ...
        @NotBlank @Email @Unique(field = UniqueFields.EMAIL) String email,
        @NotBlank @Pattern(regexp = "\\d{4,12}") @Unique(field = UniqueFields.ID_DOCUMENT) String idDocument,
        @NotBlank @Size(min = 6, max = 6) @Pattern(regexp = "^[a-zA-Z0-9]{3}[a-zA-Z0-9]{3}$") String password,
        Role role) {
}
